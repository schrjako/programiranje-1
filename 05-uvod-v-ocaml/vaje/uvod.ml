...
let rec starting_element = function
  | [] -> failwith "Prazen seznam in starting_element"
  | [ x ] -> x
  | f :: _ -> f;;
...
let rec get k arr = if k < 0 then get 0 arr else match (k, arr) with
  | (k, []) -> failwith "k out of bounds"
  | (0, arr) -> starting_element arr
  | (k, f :: tail) -> get (k-1) tail;;
...
let rec insert k x arr = if k <= 0 then x :: arr else
  match (k, arr) with
    | 0, tail -> x :: tail
    | k, [] -> [ x ]
    | k, f :: tail -> f :: insert (k-1) x tail
...
let rec divide k arr = if k <= 0 then ([], arr) else
  match (k, arr) with
    | 0, arr -> ([], arr)
    | k, [] -> ([], [])
    | k, f :: tail -> let (l, r) = divide (k-1) tail in (f :: l, r);;
...
let rec rotate n arr = let (l, r) = divide n arr in r @ l;;
...
let rec remove x arr =
  match (x, arr) with
   | x, [] -> []
   | x, [ y ] -> if y = x then [] else [y]
   | x, f :: tail -> if f = x then remove x tail else f :: remove x tail;;
...
let rec reverse = function
  | [] -> []
  | [ x ] -> [ x ]
  | f :: tail -> reverse tail @ [f]
let rec is_palindrome arr = arr = reverse arr;;
...
let rec max_on_components arr1 arr2 =
  match (arr1, arr2) with
    | [], ost | ost, [] -> []
    | f1 :: tail1, f2 :: tail2 -> max f1 f2 :: max_on_components tail1 tail2;;
...
let rec max_in_arr = function
  | [] -> failwith "No elements to choose a max from"
  | [ x ] -> x
  | f :: tail -> max f (max_in_arr tail);;
let rec second_largest arr = max_in_arr (remove (max_in_arr arr) arr)
