...
let randlist l m = List.init l (fun _ -> Random.int m)
...
let rec insert y = function
  | [] -> [y]
  | f :: tail -> if compare y f <= 0 then y :: f :: tail
  else f :: (insert y tail) 
...
let rec insert_sort = function
  | [] -> []
  | f :: tail -> insert f (insert_sort tail)
...
let min_and_rest = function
  | [] -> None
  | l -> let m = List.fold_left min (List.hd l) l in
    let rec rmfirst = function
      | [] -> failwith "haven't found requested element"
      | f :: tail when f = m -> tail
      | f :: tail -> f :: (rmfirst tail)
    in
    Some (m, rmfirst l)
...
let swap a i j = let c = a.(i) in a.(i) <- a.(j); a.(j) <- c
